...
public:
	...
	void StartTracking();
	void AprilStartTracking();
	void EndTracking();
	void AprilEndTracking();
	bool IsResultValid();
	bool GetResult(uint64* OutResult=NULL);
	bool AprilGetResult(uint64* OutResult=NULL);
...
struct FAprilOpenGLGPUProfiler : public FGPUProfiler
{
	/** Used to measure GPU time per frame. */
	FOpenGLBufferedGPUTiming ApFrameTiming;

	/** Measuring GPU frame time with a disjoint query. */
	static const int MAX_GPUFRAMEQUERIES = 4;
	FOpenGLDisjointTimeStampQuery ApDisjointGPUFrameTimeQuery[MAX_GPUFRAMEQUERIES];
	int ApCurrentGPUFrameQueryIndex;

	class FOpenGLDynamicRHI* ApOpenGLRHI;
	// count the number of beginframe calls without matching endframe calls.
	int32 ApNestedFrameCount;
	bool bApIntialized;

	uint32 ApExternalGPUTime;

	FAprilOpenGLGPUProfiler(class FOpenGLDynamicRHI* InOpenGLRHI)
:	FGPUProfiler()
,	ApFrameTiming(InOpenGLRHI, 4)
,	ApCurrentGPUFrameQueryIndex(0)
,	ApOpenGLRHI(InOpenGLRHI)
...
,	ApExternalGPUTime(0)
	...
	void InitResources()
	{
		...
		ApFrameTiming.InitResources();
		for (int32 Index = 0; Index < MAX_GPUFRAMEQUERIES; ++Index)
		{
			ApDisjointGPUFrameTimeQuery[Index].Init(ApOpenGLRHI);
			UE_LOG(LogTemp,Log,TEXT("ApDisjointGPUFrameTimeQuery[Index].Init(ApOpenGLRHI);"));
		}
	}
...
//#endif
	...
	FAprilOpenGLGPUProfiler& GetAprilGPUProfilingData() {
		return AprilGPUProfilingData;
	}
...
private:
	...
	FAprilOpenGLGPUProfiler AprilGPUProfilingData;

	friend FAprilOpenGLGPUProfiler;
