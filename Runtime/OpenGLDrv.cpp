...
void FAprilOpenGLGPUProfiler::Cleanup()
{
	UE_LOG(LogTemp,Log,TEXT("FAprilOpenGLGPUProfiler::Cleanup"));
	if (bApIntialized)
	{
		for (int32 Index = 0; Index < MAX_GPUFRAMEQUERIES; ++Index)
		{
			ApDisjointGPUFrameTimeQuery[Index].ReleaseResources();
		}

		ApFrameTiming.ReleaseResources();
		ApNestedFrameCount = 0;
		bApIntialized = false;
	}
}

void FAprilOpenGLGPUProfiler::BeginFrame(FOpenGLDynamicRHI* InRHI)
{
	int bIntApIntialized=(int)bApIntialized;
	UE_LOG(LogTemp,Log,TEXT("bApIntialized:%d"),bIntApIntialized);
	if (!bApIntialized)
	{
		bApIntialized = true;
		InitResources();
		UE_LOG(LogTemp,Log,TEXT("BeginFrame,InitResources()"));
	}
	if (ApNestedFrameCount++>0)
	{
		// guard against nested Begin/EndFrame calls.
		return;
	}
	...
	if (GNumAlternateFrameRenderingGroups == 1)
	{
			ApCurrentGPUFrameQueryIndex = (ApCurrentGPUFrameQueryIndex + 1) % MAX_GPUFRAMEQUERIES;
			ApDisjointGPUFrameTimeQuery[ApCurrentGPUFrameQueryIndex].AprilStartTracking();
	}
	
}

void FAprilOpenGLGPUProfiler::EndFrame()
{
	UE_LOG(LogTemp,Log,TEXT("FAprilOpenGLGPUProfiler::EndFrame"));
	if (--ApNestedFrameCount != 0)
	{
		// ignore endframes calls from nested beginframe calls.
		return;
	...
	if (GNumAlternateFrameRenderingGroups == 1)
	{
			UE_LOG(LogTemp,Log,TEXT("GNumAlternateFrameRenderingGroups == 1"));
			ApDisjointGPUFrameTimeQuery[ApCurrentGPUFrameQueryIndex].AprilEndTracking();
	}
	...
	uint64 GPUTiming = 0;
	uint64 GPUFreq = FOpenGLDisjointTimeStampQuery::GetTimingFrequency();
	int OldestQueryIndex = (ApCurrentGPUFrameQueryIndex + 1) % MAX_GPUFRAMEQUERIES;
	ApDisjointGPUFrameTimeQuery[OldestQueryIndex].AprilGetResult(&GPUTiming);
	UE_LOG(LogTemp,Log,TEXT("GPUTiming:%d"),GPUTiming);
	if ( ApDisjointGPUFrameTimeQuery[OldestQueryIndex].IsResultValid())
	{
		UE_LOG(LogTemp,Log,TEXT("ApDisjointGPUFrameTimeQuery[OldestQueryIndex].IsResultValid() && ApDisjointGPUFrameTimeQuery[OldestQueryIndex].GetResult(&GPUTiming)"));
		AprilGPUFrameTime = FMath::TruncToInt( double(GPUTiming) / double(GPUFreq) / FPlatformTime::GetSecondsPerCycle() );
		GLastGPUFrameTime = AprilGPUFrameTime;
	}
	else
	{
		UE_LOG(LogTemp,Log,TEXT("GPUProfiling Failed!"));
		// Keep the timing of the last frame if the query was disjoint (e.g. GPU frequency changed and the result is undefined)
		AprilGPUFrameTime = GLastGPUFrameTime;
	}
