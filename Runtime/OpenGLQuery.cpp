...
void FOpenGLBufferedGPUTiming::PlatformStaticInitialize(void* UserData)
{
	...
	if ( !GAreGlobalsInitialized )
	{
		...
		if (GIsSupported)
		{
			UE_LOG(LogTemp,Log,TEXT("FOpenGL::SupportsTimestampQueries();"));
		}
		UE_LOG(LogTemp,Log,TEXT("SetTimingFrequency(1000 * 1000 * 1000)"));
...
{
	VERIFY_GL_SCOPE();
	UE_LOG(LogTemp,Log,TEXT("FOpenGLBufferedGPUTiming::InitResources()"));
	...
	if ( GIsSupported )
	{
		UE_LOG(LogTemp,Log,TEXT("GIsSupported==true,TimestampQueryBits > 0"));
...
void FOpenGLBufferedGPUTiming::AprilStartTiming()
{
	int32 NewTimestampIndex = (CurrentTimestamp + 1) % BufferSize;
	FOpenGLRenderQuery* TimerQuery = StartTimestamps[NewTimestampIndex];
	if (!TimerQuery->bInvalidResource && !PlatformContextIsCurrent(TimerQuery->ResourceContext))
...
void FOpenGLBufferedGPUTiming::AprilEndTiming()
{
}
...
void FOpenGLDisjointTimeStampQuery::AprilStartTracking()
{
	PlatformReleaseRenderQuery(DisjointQuery, Context);
	PlatformGetNewRenderQuery(&DisjointQuery, &Context);
	UE_LOG(LogTemp,Log,TEXT("FOpenGLDisjointTimeStampQuery::AprilStartTracking()"));
	if (!PlatformContextIsCurrent(Context))
	{
		UE_LOG(LogTemp,Log,TEXT("!PlatformContextIsCurrent(Context"));
		PlatformReleaseRenderQuery(DisjointQuery, Context);
		PlatformGetNewRenderQuery(&DisjointQuery, &Context);
	}
	// Dummy query to reset the driver's internal disjoint status
	UE_LOG(LogTemp,Log,TEXT("glBeginQuery( GL_TIME_ELAPSED, DisjointQuery );"));
	FOpenGL::TimerQueryDisjoint();
	glBeginQuery(UGL_TIME_ELAPSED, DisjointQuery );

}

void FOpenGLDisjointTimeStampQuery::AprilEndTracking()
{
	UE_LOG(LogTemp,Log,TEXT("FOpenGLDisjointTimeStampQuery::AprilEndTracking()"));
	glEndQuery(UGL_TIME_ELAPSED);
	auto error = glGetError();
	UE_LOG(LogTemp,Log,TEXT("glGetError():%d"),error);
	
	bIsResultValid = true;
}

bool FOpenGLDisjointTimeStampQuery::AprilGetResult(uint64* OutResult)
{
	GLuint Result = 0;
	glGetQueryObjectuiv(DisjointQuery, GL_QUERY_RESULT_AVAILABLE, &Result);
	if (Result==GL_FALSE)
	{
		UE_LOG(LogTemp,Log,TEXT("Result==GL_FALSE"));
		bIsResultValid = false;
	...
	UE_LOG(LogTemp,Log,TEXT("&OutResult:%d"),*OutResult);
	bIsResultValid = true;
...
void FOpenGLDisjointTimeStampQuery::InitResources()
{
	...
	if ( IsSupported() )
	{
		UE_LOG(LogTemp,Log,TEXT("PlatformGetNewRenderQuery(&DisjointQuery, &Context);"));
		PlatformGetNewRenderQuery(&DisjointQuery, &Context);
	}
	UE_LOG(LogTemp,Log,TEXT("FOpenGLDisjointTimeStampQuery::InitResources()"));
	GLuint NewQuery = 0;
	glGenQueries(1, &NewQuery);
	DisjointQuery = NewQuery;
	Context = 0;
}
...
void FOpenGLDisjointTimeStampQuery::ReleaseResources()
{
	...
	if ( IsSupported() )
	{
		UE_LOG(LogTemp,Log,TEXT("PlatformReleaseRenderQuery(DisjointQuery, Context);"));
		PlatformReleaseRenderQuery(DisjointQuery, Context);
	}
}
